...
namespace Platformer_2D
{
    public sealed class ParalaxController
    {
        ...
        public ParalaxController(Transform camera, Transform back, Transform middleBack, Transform middleFront, Transform front)
        {
            _camera = camera;
            _back = back;
            _middleBack = middleBack;
            _middleFront = middleFront;
            _front = front;
            _backStartPosition = _back.transform.position;
            _middleBackStartPos = _middleBack.transform.position;
            _frontMiddleStartPosition = _middleFront.transform.position;
        ...
        public void Update()
        {
            ...
            _middleBack.position = _middleBackStartPos + (_camera.position - _cameraStartPosition) * _coef;
            _middleFront.position = _frontMiddleStartPosition + (_camera.position - _cameraStartPosition) * _midCoef;
